...
function escalasDoHinario() {
  const b = [texto("Antes de começar", { caps: true, size: 16, cor: CINZA, after: 60 }),
    ...
    texto("Hoje há hinário em Dó, em Si♭ e em Mi♭, e um próprio da trompa, em Fá; cada músico lê o do seu instrumento. A regra é simples: toque a escala do tom que está escrito no seu hinário, no hino indicado — ele já está na tonalidade do seu instrumento, e tudo soa junto. A tabela mostra a correspondência, para o instrutor conferir: o que está em Dó aparece um tom acima no hinário em Si♭, uma sexta maior acima no de Mi♭ e uma quinta acima no hinário da trompa.", { size: 19, after: 100 })];
